...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredApps = this.getFilteredApps().filter(app =>
...
const TabItem = props => {
  ...
  const activeTabBtnClassName = isActive ? 'active-tab-button' : ''
  return (
    <li>
      <button
        type="button"
        className={`button-item ${activeTabBtnClassName}`}
...
class AppItem extends Component {
  render() {
    const {eachAppItem} = this.props
    const {imageUrl, appName} = eachAppItem
    return (
      <li className="card-item-container">
        <img src={imageUrl} className="card-item-image" alt={appName} />
        <p className="card-item-app-name">{appName}</p>
      </li>
    )
  }
}
